...
import { quoteTrade, quoteTrade1x2 } from "@/lib/quote";
import {
  prepareTrade,
  prepareTrade1x2,
  type PreparedTx,
...
/** Which of the three 1X2 outcomes to trade (C2). */
export interface Trade1x2Intent {
 ...
 *
 * Branches on market kind (C2): a 1X2 market opens the three-outcome ticket
 * keyed on `Trade1x2Intent`; a binary market keeps the YES/NO ticket.
 */
export function TradePanel({
  ...
  market: AnyMarketDto;
  intent: TradeIntent | Trade1x2Intent | null;
  onClose: () => void;
}) {
  ...
  function body() {
    ...
    if (isMarket1x2(market)) {
      const i = intent as Trade1x2Intent;
      return (
        <Trade1x2Ticket
          key={`${i.outcome}-${i.action}`}
          market={market}
          intent={i}
          onClose={onClose}
        />
    ...
    const i = intent as TradeIntent;
    return (
      <TradeTicket
        key={`${i.side}-${i.action}`}
        market={market as MarketDto}
        intent={i}
        onClose={onClose}
      />
    );
...
function TradeTicket({
  ...
  // The binary ticket only renders for a binary market, so its position is a
  // binary position (narrowed from the discriminated `UserPosition`).
  const binaryPosition = position?.kind === "binary" ? position : null;
  const heldSideBase =
    binaryPosition === null
      ? null
      : side === "YES"
        ? binaryPosition.yesTokens
        : binaryPosition.noTokens;
            ...
            <div className="flex flex-col gap-0.5">
              ...
              {binaryPosition ? (
                <span className="text-muted">
                  Held:{" "}
                  <span className="tnum font-600 text-ink">
                    {fmtShares(Number(binaryPosition.yesTokens) / 1_000_000)} YES
                    · {fmtShares(Number(binaryPosition.noTokens) / 1_000_000)} NO
                  </span>
                </span>
              ) : null}
            </div>
...
const OUTCOME_1X2_ORDER = ["Team1", "Draw", "Team2"] as const;
type Trade1x2Outcome = (typeof OUTCOME_1X2_ORDER)[number];

/**
 * 1X2 trade ticket (1d, C2). Same simulate-before-sign flow as the binary
 * ticket, but the user picks ONE of three real outcomes (Team1 / Draw / Team2)
 * and buys/sells that outcome's token via `getBuy1x2/getSell1x2` (a raised
 * compute-unit budget is set inside `prepareTrade1x2`). Keeps the BUG-3
 * `state === "Trading"` gate and BUG-5 friendlyTxError decode.
 */
function Trade1x2Ticket({
  market,
  intent,
  onClose,
}: {
  market: Market1x2Dto;
  intent: Trade1x2Intent;
  onClose: () => void;
}) {
  const { address, getAuthority } = useTxAuthority();
  const toast = useToast();
  const [action, setAction] = useState<"buy" | "sell">(intent.action);
  const [outcome, setOutcome] = useState<Trade1x2Outcome>(intent.outcome);
  const [amount, setAmount] = useState<string>("50");
  const slippage = 0.01;
  const [connectOpen, setConnectOpen] = useState(false);
  ...
  const oneXTwoPosition = position?.kind === "1x2" ? position : null;
  ...
  const heldBase = oneXTwoPosition
    ? oneXTwoPosition.tokens[active.idx] ?? 0n
    : null;
  ...
  async function review() {
    ...
    try {
      const authority = await getAuthority();
      if (!authority) throw new Error("Wallet cannot sign — reconnect");
      const p = await prepareTrade1x2(authority, {
        marketId: market.id,
        configId: market.configId,
        outcome,
        action,
            ...
            <div className="flex flex-col gap-0.5">
              ...
              {oneXTwoPosition ? (
                <span className="text-muted">
                  Held:{" "}
                  <span className="tnum font-600 text-ink">
                    {fmtShares(Number(oneXTwoPosition.tokens[0]) / 1_000_000)}{" "}
                    {homeTeam} ·{" "}
                    {fmtShares(Number(oneXTwoPosition.tokens[1]) / 1_000_000)}{" "}
                    Draw ·{" "}
                    {fmtShares(Number(oneXTwoPosition.tokens[2]) / 1_000_000)}{" "}
                    {awayTeam}
                  </span>
                </span>
              ) : null}
