...
import type { Market1x2Dto, Outcome1x2 } from "@fpm/shared";
import { centsLabel, percentLabel, volumeLabel, bpsToCents } from "@/lib/format";
import type { Trade1x2Intent } from "@/components/trade/trade-panel";
import { Card } from "@/components/ui/card";
import { Button } from "@/components/ui/button";

/**
 * 1X2 outcomes list (1c, C2). One row per REAL outcome — Team1 (home), Draw,
 * Team2 (away) — each carrying its true on-chain softmax price. A single Buy
 * button per row opens the ticket pre-filled with that outcome. Mirrors the
 * binary `OutcomesList`, but there is no YES/NO inversion: each outcome is a
 * distinct buyable token.
 */
export function OutcomesList1x2({
  market,
  onTrade,
}: {
  market: Market1x2Dto;
  onTrade: (intent: Trade1x2Intent) => void;
}) {
  const outcomes: {
    label: string;
