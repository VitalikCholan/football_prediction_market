...
World Cup prediction-market AMM on Solana (TxLINE hackathon track, deadline 2026-07-19). **3-way (1X2) LMSR market** — the sole market type: users buy Team1 / Draw / Team2 positions in USDT (TxLINE devnet mint, classic SPL), prices are softmax over `q:[u64;3]` and sum to 1; a keeper resolves markets by CPI into the TxLINE oracle program (hint-and-prove: `validate_stat` → Merkle-proof-verified `bool` for the hinted outcome's predicate); winners redeem 1 token = 1 USDT. On top: a **v1 leverage layer** (`plans/leverage-v1.md`) — no-liquidation pay-for-time leverage as cash-settled binary options written by an LP-funded `LeveragePool` per market, marked to keeper-posted prices (Drift-style cumulative funding index; fee-death instead of liquidation; coverage-ratio solvency). Leverage is per-config opt-in (`max_leverage > 0`) and fully orthogonal to the LMSR spot curve.

**Docs map.** This `CLAUDE.md` is the source of truth for **shipped state** (program/keeper/indexer/web as-built, commands, architecture). `PLAN.md` = product framing + v0/v1/v2 economics roadmap (leverage-as-option, pm-AMM). `plans/SPEC.md` = consolidated **design decisions** (D-1…D-9 + resolved open questions) and **forward/unbuilt spec** (v1 leverage on-chain mechanics §2, v2 pm-AMM curve §3, deferred items §4, TxLINE on-chain reference §5). `apps/web/DESIGN_SPEC.md` = UI wireframe spec (design tokens, 7 screens). Implement to these; update them when decisions change. *(The former per-layer plans were consolidated into `plans/SPEC.md`; executed one-shot plans — refactor-lmsr-canonical, resolve-1x2, aws-cicd — were deleted after execution; see git history.)*
