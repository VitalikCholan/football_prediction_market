...
World Cup prediction-market AMM on Solana (TxLINE hackathon track, deadline 2026-07-19). **3-way (1X2) LMSR market** — the sole market type: users buy Team1 / Draw / Team2 positions in USDT (TxLINE devnet mint, classic SPL), prices are softmax over `q:[u64;3]` and sum to 1; a keeper resolves markets by CPI into the TxLINE oracle program (hint-and-prove: `validate_stat` → Merkle-proof-verified `bool` for the hinted outcome's predicate); winners redeem 1 token = 1 USDT.
