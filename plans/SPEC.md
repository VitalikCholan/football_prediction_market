...
### 3.1 — 3-way (1X2) LMSR market (v1 — on-chain BUILT, full-stack wiring UNBUILT)

> **Status (2026-07-11):** phases A+B+C **merged to main** (`41191cd`). A = `lmsr.rs` (fixed-point LMSR). B = `predicate_1x2.rs` + `plans/resolve-1x2.md` (1-of-3 protocol, EqualTo wall dissolved). **C = the on-chain market**: `Market1x2` (`q:[u64;3]`+`b`, `supply:[u64;3]`) + `Position1x2` (`tokens:[u64;3]`) + 10 `*_1x2` instructions (incl. dedicated `create_market_config_1x2`), `market_kind:u8`+`resolutionPeriod:i32` carved from `MarketConfig._reserved` (44→39 bytes, zero-default Binary, no migration), `math::assert_solvent_multi` (`vault ≥ max(supply_i)`), and the `buy_delta_for_cost` delta-solve (≤61 halvings, ~660k CU worst-case — callers MUST raise the CU limit). 100 tests green (62 amm unit + 37 LiteSVM + 1). IDL: purely additive except two Borsh-wire-identical nested-struct refactors (`create_market_config`→`FeeParamsArgs`, `resolve`→`ScoresBatchSummary`). **Remaining: phase C2 (full-stack)** — keeper (create/seed/`resolve_1x2` hint), indexer (3 prices, 3-outcome events, DTOs), web (3-chip Team1/Draw/Team2 panel). Breaking account layout → fresh 1X2 markets, not in-place. **Phase C-add — `mint_set_1x2`/`redeem_set_1x2` BUILT** (12 `*_1x2` instructions now): a complete set = `amount` tokens of every outcome, charged/paid the flat face value `amount` USDT (NOT the approximate LMSR `cost`), fee-free + slippage-free + price-neutral. Moves all three `q[i]` AND `supply[i]` by `amount` (the phase-C invariant is `q[i] = seed_q[i] + supply[i]` — buy/sell move both by the same delta, so equal shift keeps softmax prices fixed and solvency exact); `collateral` mirrors buy/sell (`+amount` mint, saturating `−amount` redeem) for D-4 Void basis. Trading-only state gate. Two new log events `SetMinted1x2`/`SetRedeemed1x2`. IDL purely additive (2 instructions). 44 LiteSVM tests (+7 for sets: round-trip lossless, price-neutral incl. skewed book, solvency, overdraw/zero-amount rejects, non-Trading reject, minted-set-redeems-to-`amount`-under-each-outcome, Void basis refund, no-arb vs separate buys).
