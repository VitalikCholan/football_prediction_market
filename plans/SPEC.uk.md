...
### 3.1 — 3-way (1X2) LMSR-ринок (v1 — on-chain BUILT, full-stack wiring UNBUILT)

> **Статус (2026-07-11):** фази A+B+C **змержені в main** (`41191cd`). A = `lmsr.rs` (fixed-point LMSR). B = `predicate_1x2.rs` + `plans/resolve-1x2.md` (протокол 1-of-3, EqualTo-стіна розчинена). **C = on-chain ринок**: `Market1x2` (`q:[u64;3]`+`b`, `supply:[u64;3]`) + `Position1x2` (`tokens:[u64;3]`) + 10 інструкцій `*_1x2` (включно з окремою `create_market_config_1x2`), `market_kind:u8`+`resolutionPeriod:i32` вирізані з `MarketConfig._reserved` (44→39 байт, zero-default Binary, без міграції), `math::assert_solvent_multi` (`vault ≥ max(supply_i)`) і delta-solve `buy_delta_for_cost` (≤61 halvings, ~660k CU worst-case — callers МУСЯТЬ підняти CU-ліміт). 100 тестів зелені (62 amm unit + 37 LiteSVM + 1). IDL: суто additive окрім двох Borsh-wire-identical nested-struct refactor'ів (`create_market_config`→`FeeParamsArgs`, `resolve`→`ScoresBatchSummary`). **Залишок: фаза C2 (full-stack)** — keeper (create/seed/`resolve_1x2` hint), indexer (3 ціни, 3-outcome events, DTO), web (3-chip панель Team1/Draw/Team2). Breaking account layout → свіжі 1X2-ринки, не in-place. **Фаза C-add — `mint_set_1x2`/`redeem_set_1x2` BUILT** (тепер 12 інструкцій `*_1x2`): complete set = `amount` токенів кожного outcome, заряд/виплата за пласким номіналом `amount` USDT (НЕ наближений LMSR `cost`), без fee + без slippage + price-neutral. Рухає всі три `q[i]` І `supply[i]` на `amount` (інваріант фази C: `q[i] = seed_q[i] + supply[i]` — buy/sell рухають обидва на той же delta, тож рівний зсув тримає softmax-ціни фіксованими й solvency точним); `collateral` дзеркалить buy/sell (`+amount` mint, saturating `−amount` redeem) для D-4 Void basis. State gate — тільки Trading. Дві нові log-події `SetMinted1x2`/`SetRedeemed1x2`. IDL суто additive (2 інструкції). 44 LiteSVM-тести (+7 на set-и: round-trip без втрат, price-neutral включно зі скошеним book, solvency, overdraw/zero-amount reject, non-Trading reject, minted-set-redeems-to-`amount`-під-кожним-outcome, Void basis refund, no-arb проти окремих buy).
