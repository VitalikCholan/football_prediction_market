...
13. Drop the TxLINE **devnet IDL** into `programs/amm/idls/txline.json`; `declare_program!(txline)`; mirror `ProofNode`/`StatTerm`/`ScoresBatchSummary`/`TraderPredicate` types (§11.1). Store `resolution_predicate` + stat keys on `MarketConfig`/`Market` (D-8). [ ]
14. `resolve` (arbitrary-CPI guard on `txline_program` + `daily_scores_merkle_roots` owner/PDA check; CPI `validate_stat`, read `Return<bool>::get()`, `require!(is_valid)`, derive outcome, state gate, `MarketResolved` event; map `RootNotAvailable`/proof errors to `ProofRejected` for keeper retry). [ ]
15. `redeem` (zero-before-transfer, flag, PDA-signed payout, Void policy D-4). [ ]
16. `close_market` (grace gate, sweep → close token acct → `close = authority`). [ ]
17. LiteSVM tests cases 9–11 with a **mock TxLINE program**. [ ]
18. Surfpool integration §10.2: real forked-TxLINE CPI + time-travel + full happy path. [ ]
19. `program_autofixer` loop on `resolve.rs`/`redeem.rs`/`close_market.rs`. [ ]
