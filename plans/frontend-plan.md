...
- **Real filter pills (SHIPPED):** `markets-browse.tsx` builds the pill row **dynamically from the loaded markets** — "All markets" + one pill per distinct `competition` (e.g. World Cup, Friendlies, from the indexer's `competition` field) + "● Live now" (`Trading`) + "Resolved", only when markets qualify. Client-side filter via a tagged union; falls back to "All markets" if the active pill vanishes on reload. The draft's decorative Group stage / Knockout / Outrights pills were dropped (the feed exposes only `FixtureGroupId`, no stage-name map).
- **No-feed hide (SHIPPED):** `fetchMarkets` filters out markets whose TxLINE feed never yields team names (`homeTeam` null → renders as "Fixture <id> · ? – ?") so they don't clutter the grid. Presentation-only — the market still exists on-chain and stays reachable by direct URL via `fetchMarket`.
...
- **Real-when-present (odds-feed-dependent, dark on devnet WC):** the **market-vs-pool spread** in `outcomes-list.tsx` (`Pool {x}¢ · Market {y}¢` per outcome) renders only when `market.marketOdds` is non-null; the detail breadcrumb shows the real `competition`; and market init reserves come from StablePrice `Pct` (50/50 fallback). All correct but inert on devnet where the WC odds feed returns `[]` (club-league guest odds work; WC fixtures don't). The odds-movement chart overlay (a StablePrice curve on the hero chart) is deferred for the same reason — it would be dead code until mainnet WC / a club-league market.
