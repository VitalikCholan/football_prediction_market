//! `close_market_1x2` — admin-gated teardown after the resolution grace
//! period (SPEC §3.1 phase C; mirror of `close_market`).
//!
...
//! 3. Anchor `close = authority` reclaims the `Market1x2` data account
//!    (secure close: data zeroed + discriminator poisoned → revival-safe).
//!
//! NOTE (mirrors the binary asymmetry, flagged in SPEC §3.1): like v0, there
//! is no on-chain Void path — `Outcome1x2::Void` is redeemable by
//! `redeem_1x2` but no instruction sets it yet (withholding-keeper recovery
//! is a follow-up `void_market` ix for BOTH kinds).
...
use crate::state::{GlobalConfig, Market1x2, Market1x2Closed, MarketConfig, MarketState};

#[derive(Accounts)]
pub struct CloseMarket1x2<'info> {
    #[account(mut)]
...
pub(crate) fn handler(ctx: Context<CloseMarket1x2>) -> Result<()> {
