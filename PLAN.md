...
> - §10 (v0/v1/v2 roadmap — leverage-as-option, pm-AMM, LeveragePool solvency) remains the live plan for v1/v2; on-chain mechanics in `plans/SPEC.md` §2 (v1 leverage) / §3 (v2 pm-AMM).
...
## 12. Known bugs (v0, post-ship — fix before demo)

Observed on the running devnet stack (2026-07-10). Both symptoms below share one root cause: **markets are seeded (`init_market` → state `Open`) but the keeper lifecycle never advances them** through `activate → freeze → resolve → close`. The UI faithfully mirrors on-chain state, so a stale on-chain state renders as a stale label.

### BUG-1 — seeded markets stuck in `Open` ("Upcoming") after kickoff
- **Symptom:** France–Morocco (fixture `18209181`) shows **"Upcoming"** although kickoff (Jul 9, 8:00 PM UTC) has passed. All 7 seeded markets are on-chain state `Open`; only Colombia–Ghana (`18179549`) reached `Closed`, because `scripts/full-circle.ts` drove it manually.
- **Root cause:** the keeper scheduler is not running continuously (or ran once for seeding only). Nothing calls `activate_market` at kickoff or `freeze_market` at final whistle. UI maps `Open → "Upcoming"` (`state-badge.tsx:23`).
- **VERIFIED (2026-07-11):** deeper than "not running." The scheduler's `FixtureSource` is `StaticFixtureSource(parseFixturesEnv(process.env.FIXTURES))` (`index.ts:74-91`) — the lifecycle is driven **only by the `FIXTURES` env var**, NOT by the live snapshot or the on-chain markets. `fetchFixtureSnapshot` feeds the seeder + indexer enrichment, not the scheduler. So **a deployed keeper with an empty `FIXTURES` drives nothing** — markets stay `Open`. (Past-kickoff itself is handled: `scheduler.ts:59` fires `now >= kickoffTs` on first tick — but only for fixtures in the env list.) Proper fix: build the scheduler's `Fixture[]` from the live snapshot or the on-chain `Open` markets' stored `kickoff_ts`/`freeze_ts`, not a static env. Deploy MUST set `FIXTURES` (ops workaround) until then.
- **Fix options:**
  1. **Manual lifecycle** — drive a fixture `activate → freeze → resolve → close` like `full-circle.ts` did for Colombia–Ghana. Needs a real TxLINE Merkle proof for that fixture (verified only for `18179549` so far).
  2. **Run keeper continuously** (`pnpm --filter @fpm/keeper start`) — scheduler activates/freezes on clock. `resolve` still needs a real finished-match proof per fixture.
  3. **Re-seed with genuinely-future fixtures** — so "Upcoming" is honest (kickoff actually ahead). The Jul-9 markets are stale because time passed since seeding.
- **Recommended for demo:** #3 for honest Upcoming cards + keep Colombia–Ghana as the Closed example; #1 on a proof-backed fixture if we want to show the live `resolve` flow.

### BUG-2 — live match not reflected as `Trading`/live
- **Symptom:** Spain–Belgium is actually being played, but the UI does not show it as **LIVE** — it renders as Upcoming, and no live score/clock appears. We have no signal that the match is in progress.
- **Root cause:** same as BUG-1 — the market is still `Open` (never `activate`d), so the detail's live score+clock (`market-header.tsx`, gated on `state === "Trading"`) and the "● Live now" filter never trigger. The indexer's score/odds enrichment only refreshes markets already in `Trading`.
- **Fix:** whatever resolves BUG-1 (keeper driving `activate` at kickoff) fixes this — a live match must be in `Trading` for the live UI to engage. Verify the keeper scheduler's kickoff detection against the seeded `kickoff_ts`, and confirm the indexer enriches score/clock once state flips to `Trading`.

### BUG-3 — placing a stake on a non-`Trading` market fails with a raw `Custom:6012`
- **Symptom:** opening the Buy panel on Spain–Belgium, entering an amount, and submitting fails with `{"InstructionError":["1",{"Custom":"6012"}]}` shown raw in the UI.
- **Root cause (two parts):**
  1. **Same as BUG-1** — `Custom:6012` = `AmmError::InvalidMarketState` ("Market is not in the required state for this action"). `buy` requires state `Trading`; the market is still `Open` (never activated), so the program correctly rejects. Fixing BUG-1 removes the reject.
  2. **Frontend gap (independent, fix regardless):** the buy/sell panel is not gated on `state === "Trading"`. It should disable trading (or show "Trading not open yet") for `Open`/`Locked`/`Resolved`/`Closed` markets, and decode program custom errors to friendly messages (map `6012 → "Market not open for trading yet"`) instead of dumping the raw `InstructionError` JSON.
- **FIXED (2026-07-11):** `trade-panel.tsx` now gates `tradable` on `state === "Trading"` only (was `Trading || Open`) and shows a state-aware reason ("Trading opens at kickoff." / "Trading is closed (…)."). The raw-error part is covered by BUG-5. Remaining: the "USDC" AMOUNT-field label (cosmetic USDT/USDC naming leftover; should read "USDT") — not yet done.

### BUG-4 — "Spain · Yes / Belgium · No" labels + missing Draw (binary vs 3-way mismatch)
- **Symptom:**
  - the trade panel labels the two outcomes **"Spain · Yes"** and **"Belgium · No"** (header "Spain — Yes"), reading as if NO = "Belgium wins";
  - **there is no Draw option in the trade panel** — yet the **market cards show a "Draw" column with odds** (e.g. "Draw 21¢"), implying draw is tradeable when it is not.
- **Root cause:** the market is **binary** (YES/NO), but football is a **3-way** result (home win / draw / away win). `NO` means *"Spain does NOT win"* — which includes a **draw**, not "Belgium wins". Mapping `NO → "Belgium · No"` is misleading: a Spain–Belgium draw settles NO, yet the label implies a Belgium win. The card's Draw column is **cosmetic** (derived/placeholder odds) with no on-chain outcome behind it — nothing to buy.
- **Inconsistency to resolve:** cards advertise 3 outcomes (Home/Draw/Away), the trade panel offers 2. Either drop the Draw column from cards (binary-honest) or move to a real 3-way market.
- **Fix (pick one):**
  1. **Relabel honestly for a binary market:** YES = "Spain to win", NO = "Spain not to win (draw or Belgium)". Keep the underlying binary predicate; just stop implying the away team.
  2. **Make the market predicate explicit** — tie YES/NO to the actual stored resolution predicate (e.g. "Spain wins" vs "Spain does not win") and render labels from it, not from `home`/`away` team names.
  3. **3-way markets** (home/draw/away) — the honest model for football, but a scope change (see §10 "3-way markets" — needs multi-outcome normalization, out of v0). Not for the demo fix.
- **Recommended:** #1 for the demo (one-line label change, no on-chain change); note #3 as the real fix in the roadmap.

### BUG-5 — raw program errors leak into the UI (no decoding)
- **Symptom:** program/CPI failures render as raw JSON in multiple places:
  - trade panel: `{"InstructionError":["1",{"Custom":"6012"}]}` (BUG-3, `AmmError::InvalidMarketState`).
  - Portfolio page: `{"InstructionError":["1",{"Custom":"6058"}]}` — this is **TxLINE program error `6058 = RateLimitExceeded`**, thrown by the "Get test USDT" faucet (`request_devnet_faucet`) because the wallet was already funded (balance shows $100). Expected oracle behavior, but shown raw.
- **Root cause:** no central error decoder. Custom error codes from two programs (AMM 6000–6024, TxLINE 6000+) are surfaced verbatim instead of mapped to human messages. `6012` and `6058` collide numerically but come from **different programs** — the decoder must key on the failing program id, not just the code.
- **Fix:** add a shared error-decoding helper (web + keeper already decode some in `apps/keeper/src/solana/errors.ts`) that maps `(programId, code) → message`, e.g. AMM `6012 → "Market not open for trading yet"`, TxLINE `6058 → "Faucet rate-limited — you already have test USDT"`. Show the friendly message in toasts/panels; keep the raw code only in a dev/details view.
- **FIXED (2026-07-11):** added `libs/shared/src/errors.ts` — `friendlyTxError(err, program?)` + `extractCustomCode(err)`, keyed on a caller-supplied `"amm" | "txline"` hint (codes collide across the two 6000-based programs). Wired into every web catch site: trade sim/send (`trade-panel.tsx` via `lib/tx.ts`), faucet (`use-faucet.ts` → the `6058` case), redeem (`positions-view.tsx`, `resolution-panel.tsx`). `lib/tx.ts` now delegates to the shared decoder (dropped the duplicate local AMM map). No raw `InstructionError` JSON reaches the UI. The keeper keeps its own richer log-attribution decoder.
